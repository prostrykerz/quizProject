...
<body>
	...
	<h3 style="color:red">Practice Mode (Score Will Not Be Recorded)</h3>
...
var startTime;
	$(document).ready(function() {
		...
		function showQuestion(index) {
			...
			if(!<%=(Boolean)infoMap.get("immediateFeedback")%>){
				html += "<a href='quizSummary.jsp?id="+<%=id%>+"'><button id=\"submit_btn\" type=\"button\" style='visibility: hidden'>Submit</button></a>";
			}
			$("#question_box").html("");
			...
			if(<%=(Boolean)infoMap.get("immediateFeedback")%>){
				$("#next_btn").click(function() {
					...
					if (index == questionArr.length-1){
						$("#next_btn").click(function(){
							document.location="quizSummary.jsp?id="+<%=id%>;
						});
					}
					else{
						$("#next_btn").click(function(){
